...
data class Value(val textualValue: String) {
    ...
    fun resolve(): Float {
        ...
        while (index < textualValue.length) {
            ...
            if (c.isDigit() || c == '_') {
                ...
                if (numStartIndex != -1) {
                    ...
                    val num =
                        textualValue.substring(numStartIndex, index).replace("_", "-").toFloat()
                    tokens.add(num)
                }
        ...
        if (numStartIndex != -1) {
            // push last digit
            val num = textualValue.substring(numStartIndex, textualValue.length).replace("_", "-")
                .toFloat()
            tokens.add(num)
        }
            ...
            for (o in ops) {
                if (tokens[i] == o) {
                    ...
                    if (o.requireRightValue() && o.requireLeftValue()) {
                        ...
                        // push to swap
                        val localIndex =
                            findIndex(tokens[i - 1], tokens[i], tokens[i + 1], list = tokenSwap)
                        for (j in 0..2) {
...
fun findIndex(vararg objs: Any, list: ArrayList<Any>) =
    list.indices.firstOrNull { i -> list[i] == objs[0] && objs.indices.none { list[i + it] != objs[it] } }
        ?: -1